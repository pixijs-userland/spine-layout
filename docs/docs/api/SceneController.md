...
The sprite's hit area is the shape of the slot's attachment — a region's quad, with its own offset, rotation and scale from the bone, or a mesh's hull — so the whole of the art the player sees answers the pointer. It ignores the clipping mask the runtime hands slot objects, and no skeleton answers a hit-test with its own art: Pixi hands an interactive ancestor's mode down to every child, and a `Spine` would otherwise claim its whole bounding box, so a hero or a popup drawn over a button would swallow the click. Only what a `button_` bone wraps is a hit target of its own.
